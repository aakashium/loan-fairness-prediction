...
def cleaning_data(input_path: str, output_path: str):
    ...
    leakage_cols = [
        "recoveries", "collectioni_recovery_fee", "total_rec_prncp",
    ...
    # Apply Filters
    q = (
        q.drop(leakage_cols + identifiers)
        .select([pl.col(c) for c in q.columns
        if (q.select(pl.col(c).null_counts()).collect().item()/2_000_000) < 0.35])
    )
    ...
    q = q.with_columns([
        ...
        .then(0)
        .otherwise(None) # Drop "Current" or "Late" loans as they are undefined
        .alias("target")
    ])
